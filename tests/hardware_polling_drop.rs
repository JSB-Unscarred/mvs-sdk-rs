//! Windows x64 + MVS SDK 真机 RAII buffer 归还测试。
...
// 验证 FrameGuard::drop 会归还一个 SDK buffer，防止 polling buffer 泄漏。
#[test]
#[ignore = "requires the MVS SDK, MVS_TEST_CAMERA_SERIAL, and TriggerMode=Off"]
fn guard_drop_returns_one_of_two_stream_buffers() -> Result<(), Box<dyn Error>> {
