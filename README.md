...
| `MV_CC_Finalize` | `Sdk::shutdown(&self) -> MvsResult<()>` | 相机 handle 和 callback 收敛后终止；失败时可重试，成功后进入进程终态。 |
...
| `MV_CC_CreateHandle` + `MV_CC_OpenDevice` | `DeviceInfo::open(&self, AccessMode) -> MvsResult<Camera>` | 创建并打开 handle；失败时尽力回滚销毁。另提供 `open_exclusive`、`open_control`。 |
...
| `MV_CC_RegisterImageCallBackEx` | `register_image_callback(FnMut(&Frame<'_>))` / `unregister_image_callback()` | 停止取流后变更注册；`Frame` 仅在调用期间有效，注销等待在途 callback。 |
...
| `MV_CC_SetEnumValueByString` / `MV_CC_SetEnumValue` / `MV_CC_GetEnumValueEx` | `set_enum`、`set_enum_value`、`get_enum`、`get_enum_info` | 符号名优先；`EnumNode` 最多保存 256 个候选值。 |
...
| `MV_CC_CloseDevice` + `MV_CC_DestroyHandle` | `Camera::close(self) -> MvsResult<()>` | owner 线程执行完整清理并返回首个错误；`Drop` 负责兜底。 |

## SDK 结构体与 Rust 结构体

| MVS SDK 结构体 | Rust 定义 | 转换与生命周期 |
| --- | --- | --- |
| `MV_CC_DEVICE_INFO_LIST` | `DeviceList` | 复制有效设备项；`iter()` 返回借用 Rust-owned 列表的迭代器。 |
...
| `MVCC_ENUMVALUE_EX` | `EnumNode` | 复制当前值和最多 256 个候选值。 |
...
- image callback 与 polling 互斥；注册、替换、注销或切换模式前停止采集。
- callback 应尽快返回并把关闭/重连交给 owner 线程；callback 内消费自身 `Camera` 会返回 `CallOrder` 并阻止 SDK shutdown。
...
- 相机关闭且 callback 退出后调用 `Sdk::shutdown`；native finalization 失败时可重试，成功后进程内不再初始化。
